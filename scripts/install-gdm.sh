...
# Builds navigator, builds unified area binary, and installs systemd service and GDM session
...
echo -e "${GREEN}═══════════════════════════════════════════════════════════${NC}"
echo -e "${GREEN}     Area Desktop Session Installer (GDM)${NC}"
echo -e "${GREEN}═══════════════════════════════════════════════════════════${NC}"
echo ""
...
if [ "$EUID" -eq 0 ]; then
    ...
    # GDM typically only looks in /usr/share/xsessions, so we need sudo for session file
    XSESSION_DIR="/usr/share/xsessions"
...
# ============================================================================
# Step 7: Install GDM Session File
# ============================================================================
echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"
echo -e "${YELLOW}Step 7: Installing GDM Session File${NC}"
echo -e "${YELLOW}═══════════════════════════════════════════════════════════${NC}"

# GDM session file must be in /usr/share/xsessions (requires sudo for user installs)
# GDM prefers desktop entries with additional metadata
TEMP_DESKTOP=$(mktemp)
...
Type=Application
DesktopNames=Area
Keywords=window;manager;compositor;area;
...
if [ "$INSTALL_TYPE" = "user" ]; then
    ...
    if sudo install -m 644 "$TEMP_DESKTOP" "$XSESSION_DIR/area.desktop" 2>/dev/null; then
        log_success "GDM session file installed (with sudo)"
    else
        log_error "Failed to install GDM session file (sudo required)"
    fi
else
    if run_cmd "Installing GDM session file" install -m 644 "$TEMP_DESKTOP" "$XSESSION_DIR/area.desktop"; then
        log_success "GDM session file installed"
    fi
fi
rm -f "$TEMP_DESKTOP"
...
if [ $AREA_CRITICAL_ERRORS -eq 0 ]; then
    ...
    echo "  ✓ GDM session: $XSESSION_DIR/area.desktop"
    ...
    if [ "$INSTALL_TYPE" = "user" ]; then
        ...
        echo "  2. Select 'Area' from the GDM session menu (gear icon)"
        echo "  3. Log in"
    else
        echo "  1. All users can now select 'Area' from GDM"
    fi
    echo ""
