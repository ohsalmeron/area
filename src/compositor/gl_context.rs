...
unsafe extern "C" fn x_error_handler(
    ...
    if !event.is_null() {
        ...
        if error_code == 4 {
            // BadPixmap
            warn!(
                "X Error BadPixmap (code=4): request={}, minor={} - GLX pixmap creation failed, will use fallback rendering",
                request_code, minor_code
            );
        } else if error_code == 8 {
            // BadMatch
            warn!(
                "X Error BadMatch (code=8): request={}, minor={} - Invalid parameter attributes, will use fallback rendering",
                request_code, minor_code
            );
        } else {
            warn!(
                "X Error: code={}, request={}, minor={}",
...
impl GlContext {
    ...
    pub fn new(_conn: &RustConnection, screen_num: usize, root: u32) -> Result<Self> {
        ...
        info!(
            "Overlay window depth: {}, visual ID: 0x{:x}",
            overlay_depth, overlay_visual_id
        );
        ...
        if overlay_visual_id != 0 {
            for i in 0..num_configs as usize {
                ...
                let vinfo = unsafe { (glx.glXGetVisualFromFBConfig)(display, test_config) };
                ...
                if !vinfo.is_null() {
                    ...
                    if config_visual_id == overlay_visual_id {
                        ...
                        info!(
                            "Found FBConfig matching overlay visual ID 0x{:x} with TFP support (perfect match)",
                            overlay_visual_id
                        );
                        break;
                    }
        ...
        let mut tfp_config: Option<glx::GLXFBConfig> = None;
        {
            ...
            if !tfp_configs_ptr.is_null() && num_tfp_configs > 0 {
                ...
                unsafe {
                    (xlib.XFree)(tfp_configs_ptr as *mut _);
                }
            }
        }
        ...
        if let Some(perfect) = visual_match_tfp {
            ...
            matched_config_tfp = true;
            info!(
                "Using perfect match: visual 0x{:x} with TFP support",
                overlay_visual_id
            );
            ...
            matched_config_tfp = true;
            warn!(
                "No visual match with TFP found. Using TFP-capable config with glXCreateWindow (visual mismatch handled by GLX)"
            );
        } else if let Some(visual_fallback) = visual_match_no_tfp {
            config = Some(visual_fallback);
            matched_config_tfp = false;
            warn!(
                "No TFP-capable FBConfig found. Using visual match without TFP - compositing will not work!"
            );
        }
        ...
        unsafe {
            (xlib.XFree)(configs_ptr as *mut _);
        }
        ...
        let vinfo = unsafe { (glx.glXGetVisualFromFBConfig)(display, config) };
        ...
        info!(
            "Attempting to use overlay window {} directly for rendering",
            root
        );
        let result = unsafe { (glx.glXMakeCurrent)(display, root as u64, context) };
        unsafe {
        ...
        if result != 0 {
            ...
            info!(
                "✓ Made GLX context current with overlay window (handle: {})",
                root
            );
        } else {
            warn!(
                "Failed to make overlay window current - visual mismatch, trying GLX window creation"
            );
            ...
            if glx_version_ok {
                info!(
                    "Creating GLX window from overlay window {} (glXCreateWindow should handle visual)",
                    root
                );
                ...
                if glx_window != 0 {
                    info!(
                        "Created GLX window: {}, attempting to make current",
                        glx_window
                    );
                    let result2 = unsafe { (glx.glXMakeCurrent)(display, glx_window, context) };
                    unsafe {
                        (xlib.XSync)(display, 0);
                    }
                    if result2 != 0 {
                        ...
                        info!(
                            "✓ Made GLX context current with GLX window (handle: {})",
                            glx_window
                        );
                    } else {
        ...
        if !make_current_success {
            error!(
                "All glXMakeCurrent attempts failed. Overlay window: {}, GLX version: {}.{}",
                root, major, minor
            );
            unsafe {
            ...
            return Err(anyhow::anyhow!(
                "glXMakeCurrent failed - could not use overlay window or create GLX window"
            ));
        }
        ...
        if bind_tex.is_none() || release_tex.is_none() {
            ...
            return Err(anyhow::anyhow!(
                "GLX_EXT_texture_from_pixmap functions missing"
            ));
        }
        ...
        if std::env::var("AREA_BENCHMARK").is_err() {
            if let Some(swap_func) = swap_fn {
                unsafe {
                    ...
                    info!(
                        "Enabling VSync (glXSwapIntervalEXT) on overlay window {}",
                        root
                    );
    ...
    pub fn make_current(&self) -> Result<()> {
        let result =
            unsafe { (self.glx.glXMakeCurrent)(self.display, self.drawable as u64, self.context) };
        if result == 0 {
    ...
    fn initialize_depth_configs(&mut self) -> Result<()> {
        ...
        for depth in [8u8, 15, 16, 24, 32] {
            ...
            for i in 0..num_configs as usize {
                ...
                // Get visual depth
                let vinfo =
                    unsafe { (self.glx.glXGetVisualFromFBConfig)(self.display, test_config) };
                if vinfo.is_null() {
                    continue;
                }
                ...
                unsafe {
                    (self.xlib.XFree)(vinfo as *mut _);
                }
            ...
            if let Some(config) = best_config {
                ...
                info!(
                    "Found optimal FBConfig for depth {}: mipmap={}, y_inverted={}",
        ...
        unsafe {
            (self.xlib.XFree)(configs_ptr as *mut _);
        }
        Ok(())
    }
    ...
    fn verify_pixmap(&self, pixmap: u32) -> Result<()> {
        ...
        unsafe {
            ...
            if result == 0 || had_error {
                ...
                X_ERROR_CODE.store(0, Ordering::Relaxed);
                return Err(anyhow::anyhow!(
                    "Pixmap {} verification failed: X error code {} (pixmap may not exist)",
                    pixmap,
                    error_code
                ));
            }
            ...
            if width_return == 0 || height_return == 0 {
                ...
                X_ERROR_CODE.store(0, Ordering::Relaxed);
                return Err(anyhow::anyhow!(
                    "Pixmap {} has invalid dimensions {}x{}",
                    pixmap,
                    width_return,
                    height_return
                ));
    ...
    pub fn create_glx_pixmap(&self, pixmap: u32, depth: u8) -> Result<u32> {
        ...
        if glx_pixmap == 0 || had_error {
            let error_msg = if error_code == 4 {
                format!(
                    "BadPixmap - pixmap {} is invalid or incompatible with GLX config",
                    pixmap
                )
            } else if error_code == 8 {
                format!(
                    "BadMatch - pixmap {} has invalid parameter attributes for GLX config",
                    pixmap
                )
            } else if had_error {
                format!(
                    "X Error code {} - pixmap {} GLX creation failed",
                    error_code, pixmap
                )
            } else {
                format!(
                    "glXCreatePixmap returned 0 - pixmap {} may be incompatible with GLX config",
                    pixmap
                )
            };
