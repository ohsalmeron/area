...
impl MoveResizeManager {
    ...
    pub fn start_resize(
        ...
        debug!(
            "Starting resize operation for window {} (direction={:?})",
            window, direction
        );
    ...
    pub fn handle_motion(
        ...
        match state.operation {
            MoveResizeOperation::Move => {
                ...
                new_x = new_x
                    .min(work_area.x + work_area.width as i32 - state.start_geometry.width as i32);
                new_y = new_y.min(
                    work_area.y + work_area.height as i32 - state.start_geometry.height as i32,
                );
                ...
                conn.configure_window(target_window, &ConfigureWindowAux::new().x(new_x).y(new_y))?;
            ...
            MoveResizeOperation::Resize(direction) => {
                ...
                match direction {
                    ResizeDirection::TopLeft => {
                        ...
                        new_geom.width = ((state.start_geometry.width as i32)
                            .saturating_sub(dx as i32))
                        .max(1) as u32;
                        new_geom.height = ((state.start_geometry.height as i32)
                            .saturating_sub(dy as i32))
                        .max(1) as u32;
                    }
                    ResizeDirection::Top => {
                        new_geom.y = state.start_geometry.y + dy as i32;
                    ...
                    ResizeDirection::TopRight => {
                        new_geom.y = state.start_geometry.y + dy as i32;
                        new_geom.width =
                            ((state.start_geometry.width as i32) + dx as i32).max(1) as u32;
                        new_geom.height = ((state.start_geometry.height as i32)
                            .saturating_sub(dy as i32))
                        .max(1) as u32;
                    }
                    ...
                    ResizeDirection::BottomRight => {
                        new_geom.width =
                            ((state.start_geometry.width as i32) + dx as i32).max(1) as u32;
                        new_geom.height =
                            ((state.start_geometry.height as i32) + dy as i32).max(1) as u32;
                    }
