...
pub fn handle_net_moveresize_window(
    ...
    debug!(
        "_NET_MOVERESIZE_WINDOW: window={}, flags={:x}, x={}, y={}, w={}, h={}",
    ...
    if let Some(client) = clients.get_mut(&window) {
        ...
        if client.is_maximized() && (flags & USER_POS) == 0 {
            debug!(
                "Refusing to move maximized window {} without USER_POS flag",
                window
            );
            return Ok(());
        }
...
pub fn handle_net_wm_moveresize(
    ...
    debug!(
        "_NET_WM_MOVERESIZE: window={}, root_x={}, root_y={}, direction={}",
    ...
    if let Some(_client) = clients.get(&window) {
        ...
        debug!(
            "_NET_WM_MOVERESIZE not yet fully implemented for window {}",
            window
        );
    } else {
...
pub fn handle_net_wm_fullscreen_monitors(
    ...
    debug!(
        "_NET_WM_FULLSCREEN_MONITORS: window={}, top={}, bottom={}, left={}, right={}",
...
pub fn update_client_list_stacking(
    ...
    client_list.retain(|&w| clients.get(&w).map(|c| c.mapped()).unwrap_or(false));
    ...
    debug!(
        "Updated _NET_CLIENT_LIST_STACKING with {} windows",
        client_list.len()
    );
