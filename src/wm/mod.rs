...
impl WindowManager {
    ...
    pub fn new(
        ...
        let wm_selection_atom = conn
            .intern_atom(false, wm_selection_name.as_bytes())?
            .reply()
        ...
        let current_wm_owner = conn
            .get_selection_owner(wm_selection_atom)?
            .reply()
        ...
        if current_wm_owner != 0 {
            ...
            info!(
                "Existing WM detected (window 0x{:x}), attempting replace...",
                current_wm_owner
            );
        ...
        conn.set_selection_owner(wm_owner_window, wm_selection_atom, x11rb::CURRENT_TIME)?
            .check()
        ...
        if owner_after != wm_owner_window {
            anyhow::bail!(
                "Failed to acquire WM selection ownership (expected 0x{:x}, got 0x{:x})",
                wm_owner_window,
                owner_after
            );
        }
        ...
        if current_wm_owner != 0 {
            ...
            while start.elapsed() < timeout {
                ...
                if current_owner == wm_owner_window {
                    ...
                    if conn
                        .get_window_attributes(current_wm_owner)?
                        .reply()
                        .is_err()
                    {
        ...
        debug!(
            "WM: Current root event mask: {:?}",
            root_attrs.your_event_mask
        );
        ...
        conn.flush()?;
        debug!(
            "WM: _NET_SUPPORTING_WM_CHECK set to window 0x{:x}",
            wm_owner_window
        );
    ...
    fn should_decorate_from_patterns<C: Connection>(
        ...
        if res_class.contains("google-chrome")
            || res_class.contains("chromium")
            || res_name.contains("chrome")
            || res_name.contains("chromium")
        {
            return Ok(false);
        }

        // Check for Firefox
        if res_class.contains("firefox")
            || res_class.contains("navigator")
            || res_name.contains("firefox")
            || res_name.contains("navigator")
        {
            return Ok(false);
        }

        // Check for Electron apps
        if res_class.contains("electron")
            || res_name.contains("electron")
        ...
        // Check for Wine apps
        if res_class.contains("wine") || res_name.contains("wine") || title_lower.contains(".exe") {
            return Ok(false);
        }
        ...
        if title_lower.contains("steam")
            && (title_lower.contains("game") || title_lower.contains("launch"))
        {
            return Ok(false);
        }
    ...
    pub fn manage_window(
        ...
        let attrs = match conn.get_window_attributes(client.window)?.reply() {
            ...
            Err(e) => {
                debug!(
                    "WM: Failed to get attributes for window {}, it probably disappeared: {}",
                    client.window, e
                );
                return Ok(());
            }
        ...
        let geom = match conn.get_geometry(client.window)?.reply() {
            ...
            Err(e) => {
                debug!(
                    "WM: Failed to get geometry for window {}, it probably disappeared: {}",
                    client.window, e
                );
                return Ok(());
            }
        ...
        let has_position_hint = if let Ok(reply) = conn
            .get_property(
                false,
                client.window,
                AtomEnum::WM_NORMAL_HINTS,
        ...
        if let Ok(Some(motif_should_decorate)) = self
            .atoms
            .should_decorate_from_motif_hints(conn, client.window)
        {
            should_decorate = motif_should_decorate;
            debug!(
                "MOTIF hints for window {}: should_decorate={}",
                client.window, should_decorate
            );
        } else {
            // MOTIF hints not present or don't specify - check _NET_WM_WINDOW_TYPE
            let window_types = self
                .atoms
                .get_window_type(conn, client.window)
                .unwrap_or_default();
            ...
            if should_decorate {
                should_decorate = Self::should_decorate_from_patterns(
                    conn,
                    client.window,
                    &client.name.as_str(),
                )?;
                if !should_decorate {
                    debug!(
                        "Window {} matched no-decoration pattern (class/name)",
                        client.window
                    );
                }
            }
        ...
        if should_decorate {
            ...
            let _ = self
                .atoms
                .update_frame_extents(conn, client.window, 2, 2, 32, 2);
        } else {
            // If not decorated, set _NET_FRAME_EXTENTS to 0
            let _ = self
                .atoms
                .update_frame_extents(conn, client.window, 0, 0, 0, 0);
    ...
    pub fn unmanage_window(
        ...
        if let Some(frame_state) = &client.frame {
            ...
            if let Err(err) = frame.destroy(conn, screen.root) {
                warn!(
                    "Failed to destroy frame for window {}: {}",
                    client.window, err
                );
            }
            client.frame = None;
    ...
    pub fn close_window(&mut self, conn: &RustConnection, window_id: u32) -> Result<()> {
    ...
    pub fn toggle_maximize(
        ...
        let client = windows.get_mut(&window_id).context("Window not found")?;
    ...
    /// Maximize window
    pub fn maximize_window(&mut self, conn: &RustConnection, client: &mut Client) -> Result<()> {
        ...
        client
            .flags
            .insert(crate::wm::client_flags::ClientFlags::MAXIMIZED_VERT);
        client
            .flags
            .insert(crate::wm::client_flags::ClientFlags::MAXIMIZED_HORIZ);
        ...
        if let Some(frame_state) = &client.frame {
            ...
            frame.resize(
                ...
                frame_width as u16,
                frame_height as u16,
                &crate::config::WindowDecorationConfig {
                    titlebar_height: 32,
                    border_width: 2,
                    button_size: 20,
                    button_padding: 5,
        ...
        // Update EWMH state
        if let Err(e) = self.atoms.set_window_state(
            conn,
            client.window,
            &[
            ...
            warn!(
                "Failed to update maximized state for window {}: {}",
                client.window, e
            );
        }
    ...
    pub fn set_fullscreen(
        ...
        if fullscreen {
            ...
            client
                .flags
                .insert(crate::wm::client_flags::ClientFlags::FULLSCREEN);
            ...
            client
                .flags
                .remove(crate::wm::client_flags::ClientFlags::FULLSCREEN);
            ...
            if let Some(restore) = client.restore_geometry() {
                ...
                warn!(
                    "Cannot restore window {} - no saved geometry found",
                    client.window
                );
            }
    ...
    pub fn restore_window(&mut self, conn: &RustConnection, client: &mut Client) -> Result<()> {
        ...
        client
            .flags
            .remove(crate::wm::client_flags::ClientFlags::MAXIMIZED_VERT);
        client
            .flags
            .remove(crate::wm::client_flags::ClientFlags::MAXIMIZED_HORIZ);
        client.set_restore_geometry(None);
    ...
    pub fn minimize_window(
        ...
        client
            .flags
            .insert(crate::wm::client_flags::ClientFlags::ICONIFIED);
    ...
    pub fn set_focus(
        ...
        if let Some(client) = windows.get_mut(&window_id) {
            ...
            self.atoms
                .update_active_window(conn, self.root, Some(window_id))?;
    ...
    pub fn start_drag(
        ...
        let client = windows.get(&window_id).context("Window not found")?;

        info!(
            "Starting drag for window {} at root coordinates ({}, {})",
            window_id, start_x, start_y
        );
    ...
    pub fn update_drag(
        ...
        if let Some(ref drag) = self.drag_state {
            ...
            if let Some(frame) = &client.frame {
                ...
                conn.configure_window(client.window, &ConfigureWindowAux::new().x(new_x).y(new_y))?;
            }
