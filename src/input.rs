...
use x11rb::protocol::xproto::{Atom, ConnectionExt as XProtoExt, PropMode};
...
impl InputManager {
    ...
    pub fn new(conn: Arc<RustConnection>) -> Result<Self> {
        ...
        let version_reply = version_cookie
            .reply()
            .context("Failed to query XInput version")?;

        info!(
            "XInput extension version: {}.{}",
        ...
        let float_atom = conn
            .as_ref()
            .intern_atom(false, b"FLOAT")?
            .reply()
            .context("Failed to intern FLOAT atom")?
            .atom;

        // Try to intern libinput atoms (may fail if libinput not available)
        let accel_speed_atom = conn
            .as_ref()
            .intern_atom(false, b"libinput Accel Speed")
            .ok()
            .and_then(|cookie| cookie.reply().ok())
            .map(|reply| reply.atom);

        let accel_profile_atom = conn
            .as_ref()
            .intern_atom(false, b"libinput Accel Profile Enabled")
            .ok()
            .and_then(|cookie| cookie.reply().ok())
            .map(|reply| reply.atom);

        let left_handed_atom = conn
            .as_ref()
            .intern_atom(false, b"libinput Left Handed Enabled")
            .ok()
            .and_then(|cookie| cookie.reply().ok())
            .map(|reply| reply.atom);
    ...
    pub fn list_pointer_devices(&self) -> Result<Vec<u16>> {
        ...
        let devices_reply = devices_cookie
            .reply()
        ...
        for device_info in &devices_reply.devices {
            ...
            if is_pointer {
                ...
                debug!(
                    "Found pointer device: id={}, use={:?}",
                    device_id, device_info.device_use
                );
            }
        }
    ...
    pub fn set_libinput_accel_speed(&self, device_id: u16, speed: f32) -> Result<()> {
        ...
        debug!(
            "Setting libinput Accel Speed to {} for device {}",
            speed, device_id
        );

        use x11rb::protocol::xinput::{XIChangePropertyAux, xi_change_property};
    ...
    pub fn set_libinput_accel_profile(&self, device_id: u16, profile: &str) -> Result<()> {
        ...
        debug!(
            "Setting libinput Accel Profile to {:?} for device {}",
            profile_array, device_id
        );

        // Intern INTEGER atom
        let integer_atom = self
            .conn
            .as_ref()
            .intern_atom(false, b"INTEGER")?
            .reply()
    ...
    pub fn set_libinput_left_handed(&self, device_id: u16, enabled: bool) -> Result<()> {
        ...
        debug!(
            "Setting libinput Left Handed Enabled to {} for device {}",
            enabled, device_id
        );
        ...
        for device_id in devices {
            ...
            if let Some(speed) = config.accel_speed {
                if let Err(e) = self.set_libinput_accel_speed(device_id, speed) {
                    warn!(
                        "Failed to set acceleration speed for device {}: {}",
                        device_id, e
                    );
                    fail_count += 1;
                    continue;
            ...
            if let Some(ref profile) = config.accel_profile {
                if let Err(e) = self.set_libinput_accel_profile(device_id, profile) {
                    warn!(
                        "Failed to set acceleration profile for device {}: {}",
                        device_id, e
                    );
                    fail_count += 1;
                    continue;
            ...
            if let Some(left_handed) = config.left_handed {
                if let Err(e) = self.set_libinput_left_handed(device_id, left_handed) {
                    warn!(
                        "Failed to set left-handed mode for device {}: {}",
                        device_id, e
                    );
                    fail_count += 1;
                    continue;
